...
suppressMessages(library(gbm, warn.conflicts = FALSE, quietly=TRUE))
...
Random forest has an accuracy of `r confusionMatrix(predict.rf, testing.part$classe)$overall['Accuracy']`, boosted trees have an accuracy of `r confusionMatrix(predict.gbm, testing.part$classe)$overall['Accuracy']` and linear discriminant analysis has an accuracy of `r confusionMatrix(predict.lda, testing.part$classe)$overall['Accuracy']`. Therefore we will use the Random Forest model.
